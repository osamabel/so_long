...
/*   Updated: 2022/01/17 22:19:31 by obelkhad         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../so_long_header.h"

void	ckeck_enimy_axes_enimy(t_data *data, int X, int Y)
{
	int j;
	...
	while (j < data->enimy)
	{
		if (Y == data->enimy_axes[j].y && X == data->enimy_axes[j].x)
		{
...
void	check_box_enimy(t_data *data)
{
	...
	if ((!ft_strcmp(data->arrows_enimy ,"left") || !ft_strcmp(data->arrows_enimy ,"right")))
		left_right_enimy(data);
	if ((!ft_strcmp(data->arrows_enimy ,"up") || !ft_strcmp(data->arrows_enimy ,"down")))
		up_down_enimy(data);
	free(data->arrows_enimy);
}